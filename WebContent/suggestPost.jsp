   ...
   <!-- Post-->
    <%
     ...
     <div style="margin-top:25px" class="col-lg-9">
            ...
            <c:set var="writer" value="<%=vo.getId()%>"/>
               <c:choose>
               	...
               	<c:when test="${sessionId eq writer}">
                        	<button class="btn  btn-dark float-right" onclick="location.href='http://localhost:8080/hunminjeomum/PostServlet?cmd=suggest_delete&num=<%=vo.getPostNum()%>&date=<%=vo.getDate() %>'">삭제</button>
                        	<button class="btn  btn-dark float-right" onclick="location.href='http://localhost:8080/hunminjeomum/PostServlet?cmd=suggest_update&num=<%=vo.getPostNum()%>&date=<%=vo.getDate() %>'">수정</button>       
               				<br><br>
               </c:when>
               	<c:when test="${sessionId eq 'admin'}">
                        	<button class="btn  btn-dark float-right" onclick="location.href='http://localhost:8080/hunminjeomum/PostServlet?cmd=suggest_delete&num=<%=vo.getPostNum()%>&date=<%=vo.getDate() %>'">삭제</button>    
        ...
        <br>
        <!-- 댓글 부분 -->
        <form action="http://localhost:8080/hunminjeomum/PostServlet?cmd=suggest_comment_add&id=<%=sessionId%>&num=<%=vo.getPostNum() %>" method="post">
    	<div id="comment">
               ...
               <c:choose>
               	...
               	<c:when test="${sessionId eq writer}">
					<td width="100" align="center">  
						<button type="button" class="btn  btn-dark" onclick="location.href='http://localhost:8080/hunminjeomum/PostServlet?cmd=suggest_comment_delete&id=<%=sessionId%>&num=<%=vo.getPostNum()%>&date=<%=commentVO.getDate()%>'">삭제</button>
                	</td>
               </c:when>
               	<c:when test="${sessionId eq 'admin'}">
                	<td width="100" align="center">
                		<button type="button" class="btn  btn-dark" onclick="location.href='http://localhost:8080/hunminjeomum/PostServlet?cmd=suggest_comment_delete&id=<%=commentVO.getId()%>&num=<%=vo.getPostNum()%>&date=<%=commentVO.getDate()%>'">삭제</button>
               		 </td>
               	</c:when>
               	<c:otherwise>
               		<td width="100">        
